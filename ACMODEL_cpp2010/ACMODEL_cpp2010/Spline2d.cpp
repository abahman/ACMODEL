...
//new class
/*RefPropSPthPT refcritthPT("./R410Aprop/r410acritth.tbl");
RefPropSPtrPT refcrittrPT("./R410Aprop/r410acrittr.tbl");
RefPropSPthPH refcritthPH("./R410Aprop/r410acritth.tbl");*/
...
void Spline2d::AllocateMemory()
{
	...
	for(int i=0;i<nProp+2;i++) TBL[i] = new double[nData];

	y2 = new double **[nProp];
	for(int i=0;i<nProp;i++) {
		y2[i] = new double*[nX];
		for(int j=0;j<nX;j++) y2[i][j] = new double [nY];
	}

	y = new double**[nProp];
	for(int i=0;i<nProp;i++) y[i] = new double*[nX];
...
void Spline2d::RestoreMemory()
{
	...
	if(TBL) {
		for(int i=0;i<nProp+2;i++) delete TBL[i];
		delete TBL;
	}

	if(y2) {
		for(int i=0;i<nProp;i++) {
			for(int j=0;j<nX;j++) delete y2[i][j];
			delete y2[i];
		}
		delete y2;
	}
	...
	if(y) {
		for(int i=0;i<nProp;i++) delete y[i];
		delete y;
	}
