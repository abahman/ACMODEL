...
Spline1d::AllocateMemory()
{
	...
	for(int i=0;i<nProp+1;i++) y[i] = new double[nData];
	y2 = new double*[nProp];
	for(int i=0;i<nProp;i++) y2[i] = new double[nData];
}
...
Spline1d::RestoreMemory()
{
	...
	if(y) {
		for(int i=0;i<nProp+1;i++) delete y[i];
		delete y;
	}
	if(y2) {
		for(int i=0;i<nProp;i++) delete y2[i];
		delete y2;
	}
}
